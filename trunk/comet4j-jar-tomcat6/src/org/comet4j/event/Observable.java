...
/**
 * 被观察对象，此对象线程安全，内部实现使用乐观锁机制，即get方法不加锁，add,remove会有锁
 * 职责：管理事件源对象、代理履行事件源职责
...
public class Observable<E extends Event ,L extends ListenerInterface<E>> {
	...
	/**
	 * 增加一个事件拦截器
	 * 此拦截器的interceptor方法在本对象所有事件发生之前执行，如有多个拦截器则按倒序逐个执行。
	...
	/**
	 * 事件拦截
	 * 此方法在此对象所有事件发生之前执行，可以拦截发生在对象上的所有事件，默认不做任何处理，只返回true，子类可以重写此方法进行特殊处理。
