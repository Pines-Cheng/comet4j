...
public class ExpiresCache {
	...
	private  Map<CometConnection,List<CometMessage>> cache = Collections.synchronizedMap(new HashMap<CometConnection,List<CometMessage>>());
	...
	class CacheCleaner implements Runnable {
    	...
    	private void checkExpires(){
    		CometContext.getInstance().log("缓存元素数量："+cache.size());
